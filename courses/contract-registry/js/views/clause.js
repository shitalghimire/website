...
export default function clause(view, { args, params, ctx, data }) {
  ...
  if (tpls.length) side.append(h('section.side-card', h('h3.side-card__h', 'Letters that use it'), h('ul.side-links', tpls.map((t) => h('li', h('a', { href: `#/write/${t.id}` }, icon('pen'), t.title))))));
