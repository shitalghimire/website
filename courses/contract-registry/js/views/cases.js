...
export default function cases(view, { args, ctx, data }) {
  ...
  view.append(
    ...
    /* the doors carry real figures, so their words come from the sealed file */
    h('div.case-doors', (data.doors || []).map((d) => h(`a.door.door--${d.cls}`, { href: d.href },
      h('span.door__ic', icon(d.icon)),
      h('span.door__body',
        h('span.eyebrow', d.eyebrow),
        h('h3.door__t', d.title),
        h('p.door__s', d.sub)),
      icon('arrow')))),
